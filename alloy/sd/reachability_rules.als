...
fact{
	...
	all c1: CompositeState | let n1 = getAllNodeInSameAndDeeperLevel[c1], n2 = Node - c1 - n1 | no c1.inner && n1 & n2.flowto_triggerwith[Trigger] = none => c1 in n2.flowto_triggerwith[Trigger]	
	all c1: CompositeState, r1:Region | let n1 = getAllNodeInSameAndDeeperLevel[c1.inner], n2 = Node - c1 - n1 | r1 in c1.inner && n1 & n2.flowto_triggerwith[Trigger] = none => c1 in n2.flowto_triggerwith[Trigger]
	// If only history entry, there must be a start state, because history nodes have no record at the first entry
	all c1: CompositeState | let n1 = c1.contains | no c1.inner && n1 & (Node - c1 - n1).flowto_triggerwith[Trigger] in History => #(StartState & n1) = 1
	...
	all disj r1, r2: Region, c1: CompositeState | let n1 = getAllNodeInSameAndDeeperLevel[r1] | (r1 + r2) in c1.inner && n1 & (Node - c1 - n1 - ForkNode).flowto_triggerwith[Trigger] != none => StartState & r2.contains != none
	// If a composite without regions has a standard entry, there must be a start state in it.
	all c1: CompositeState | no c1.inner && c1 in Node.flowto_triggerwith[Trigger] => #(StartState & c1.contains) = 1
	// If a composite with regions has a standard entry, there must be a start state in each region.
	all c1: CompositeState, r1:Region | r1 in c1.inner && c1 in Node.flowto_triggerwith[Trigger] => #(StartState & r1.contains) = 1 
}
