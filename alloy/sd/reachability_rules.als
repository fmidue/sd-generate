...
pred atLeastOneEntryToCompositeStates{
	all h1: HierarchicalStates | let n1 = getAllNodesInSameAndDeeperLevels[h1], n2 = Nodes - h1 - n1 | 
		h1 in n2.flow[Triggers] || some (n1 & n2.flow[Triggers]) // Standard, direct or history entry
	all rs1: RegionsStates | let n1 = getAllNodesInSameAndDeeperLevels[rs1.inner], n2 = Nodes - rs1 - n1 | 
...
fact{
	...
	all h1: HierarchicalStates, h2: HistoryNodes | let n1 = h1.h_contains | 
		no h2.flow && h2 in n1 & (Nodes - h1 - n1).flow[Triggers] => one (StartStates & n1)
	all r1: Regions, h1: HistoryNodes | let n1 = r1.r_contains | 
		no h1.flow && h1 in n1 & (Nodes - n1).flow[Triggers] => one (StartStates & n1)
	...
	all disj r1, r2: Regions, rs1: RegionsStates | let n1 = getAllNodesInSameAndDeeperLevels[r1] | 
		(r1 + r2) in rs1.inner && some (n1 & (Nodes - rs1 - n1 - ForkNodes).flow[Triggers]) =>
			one (StartStates & r2.r_contains)
	// If a composite without regions has a standard entry, there must be a start state in it.
	all h1: HierarchicalStates | h1 in Nodes.flow[Triggers] => 
	...
	all rs1: RegionsStates, r1:Regions | 
		r1 in rs1.inner && rs1 in Nodes.flow[Triggers] => 
