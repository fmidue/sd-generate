...
pred forkNodesGoToDistinctParalleRegions{
	...
	all f1: ForkNodes, r1: Regions, disj n1, n2: Nodes | 
		(n1 + n2) in r1.contains and n1 in (Flows <: from).f1.to
			implies n2 not in (Flows <: from).f1.to
}
...
pred joinNodesComeFromDistinctParalleRegions{
	all j1: JoinNodes | not lone n1: Nodes | n1 in Regions.contains and j1 in (Flows <: from).n1.to // Each join node has at least two coming arrows from regions.
	all j1: JoinNodes, r1: Regions, disj n1, n2: Nodes | 
		(n1 + n2) in r1.contains and j1 in (Flows <: from).n1.to 
			implies j1 not in (Flows <: from).n2.to
}

fact{
	all n1: ForkNodes + JoinNodes |
		(n1 in (Flows <: from).StartNodes.to implies from.n1.label = EmptyTrigger) // If such a node(frok and join) is reached from a start state, it is not left by an arrow with non-empty transition label.
		and (to.n1.label = EmptyTrigger or from.n1.label = EmptyTrigger) // No such node is both entered and left by arrows with non-empty transition label.、
