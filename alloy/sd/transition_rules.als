...
fact{
	...
	all s1: States | one s1.flow[EmptyTriggers] => no s1.flow[TriggerNames]
	...
	all c1: CompositeStates, t1: Triggers | 
		some c1.flow[t1] => 
			no (getAllNodesInSameAndDeeperLevels[c1] + getAllNodesInSameAndDeeperLevels[c1.inner]).flow[t1]
}
