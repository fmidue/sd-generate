...
fact{
	...
	all h1: HistoryNodes, hs1: HierarchicalStates | 
		let n1 = getAllNodesInSameAndDeeperLevels[hs1] | 
			h1 in hs1.h_contains => 
			{	
				no h1.flow[Triggers] || h1.flow[Triggers] in n1 // A history should be directed to a same or a deeper level
				...
				h1 not in (n1 - getStartStates[hs1]).flow[Triggers] // History should never be reached from (somewhere, possibly nested) inside their own composite states excluding start states
			}
	all h1: HistoryNodes, r1: Regions | 
		let n1 = getAllNodesInSameAndDeeperLevels[r1] | 
			...
			{	
				...
				h1 not in (n1 - getStartStates[r1]).flow[Triggers] // History should never be reached from (somewhere, possibly nested) inside their own regions excluding start states	
			}

	all h1: HistoryNodes | h1 in getAllContainedNodes // No history nodes are at the outermost level of a state diagram
