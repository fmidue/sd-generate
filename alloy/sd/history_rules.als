...
pred atMostOneDeepAndShallowHistory{
	...
	all c1: CompositeStateWithoutRegion | lone h1: ShallowHistory | h1 in c1.contains // In composite states, there is at most one shallow history.
	all r1: Region| lone h1: DeepHistory | h1 in r1.contains // In regions, there is at most one deep history.
	all c1: CompositeStateWithoutRegion | lone h1: DeepHistory | h1 in c1.contains // In composite states, there is at most one deep history.
}
...
fact{
	...
	all h1: History, c1: CompositeStateWithoutRegion | let n1 = getAllNodeInSameAndDeeperLevel[c1] | h1 in c1.contains => (h1.flowto_triggerwith[Trigger] = none || h1.flowto_triggerwith[Trigger] in n1)
																			&& c1.contains & (NormalState + CompositeStateWithoutRegion + CompositeStateWithRegion) != none // It excludes "https://github.com/fmidue/ba-zixin-wu/blob/master/examples/MyExample5.svg"
	...
	all h1: History, r1: Region | let n1 = getAllNodeInSameAndDeeperLevel[r1] | h1 in r1.contains => (h1.flowto_triggerwith[Trigger] = none || h1.flowto_triggerwith[Trigger] in n1)
																	&& r1.contains & (NormalState + CompositeStateWithoutRegion + CompositeStateWithRegion) != none // It excludes "https://github.com/fmidue/ba-zixin-wu/blob/master/examples/MyExample5.svg"
	...
	no h1: History | h1 not in CompositeStateWithoutRegion.contains + Region.contains // No history nodes are at the outermost level of a state diagram
