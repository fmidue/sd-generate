...
pred atMostOneDeepAndShallowHistoryNode{
...
fact{
	...
	all h1: HistoryNodes, hs1: HierarchicalStates | let n1 = getAllNodesInSameAndDeeperLevel[hs1] | 
		h1 in hs1.h_contains => 
			(no h1.flowto_triggerwith[Names] || h1.flowto_triggerwith[Names] in n1)
			...
			&& h1 not in (n1 - (StartStates & hs1.h_contains)).flowto_triggerwith[Names] // History should never be reached from (somewhere, possibly nested) inside their own composite states excluding start states
	all h1: HistoryNodes, r1: Regions | let n1 = getAllNodesInSameAndDeeperLevel[r1] | 
		h1 in r1.r_contains =>
			(no h1.flowto_triggerwith[Names] || h1.flowto_triggerwith[Names] in n1)
			...
			&& h1 not in (n1 - (StartStates & r1.r_contains)).flowto_triggerwith[Names] // History should never be reached from (somewhere, possibly nested) inside their own regions excluding start states	
	...
	atMostOneDeepAndShallowHistoryNode // In composite states and regions, there is at most one shallow history and at most one deep history
}
