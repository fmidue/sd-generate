...
fact{
	...
	all h1: History, hs1: HierarchicalState | let n1 = getAllNodeInSameAndDeeperLevel[hs1] | h1 in hs1.h_contains => (h1.flowto_triggerwith[Trigger] = none || h1.flowto_triggerwith[Trigger] in n1)
																			&& hs1.h_contains & (NormalState + CompositeState) != none // It excludes "https://github.com/fmidue/ba-zixin-wu/blob/master/examples/MyExample5.svg"
																			&& h1 not in (n1 - (StartState & hs1.h_contains)).flowto_triggerwith[Trigger] // History should never be reached from (somewhere, possibly nested) inside their own composite states excluding start states
	all h1: History, r1: Region | let n1 = getAllNodeInSameAndDeeperLevel[r1] | h1 in r1.r_contains => (h1.flowto_triggerwith[Trigger] = none || h1.flowto_triggerwith[Trigger] in n1)
																	&& r1.r_contains & (NormalState + CompositeState) != none // It excludes "https://github.com/fmidue/ba-zixin-wu/blob/master/examples/MyExample5.svg"
																	&& h1 not in (n1 - (StartState & r1.r_contains)).flowto_triggerwith[Trigger] // History should never be reached from (somewhere, possibly nested) inside their own regions excluding start states	
