...
pred noSameTriggersInSameLevels{
	all disj s1, s2: States, h1: HierarchicalStates | 
		(s1 + s2) in h1.h_contains => disj[s1.name, s2.name] // In a composite state without regions, no states have same names
	all disj s1, s2: States, r1: Regions | 
		(s1 + s2) in r1.r_contains  => disj[s1.name, s2.name] // In a region, no states have same names
	...
	all disj r1, r2: Regions, c1: RegionsStates |
		(r1 + r2) in c1.inner => disj[r1.name, r2.name] // In a composite state, no regions have same names
}
...
pred outermostLevelsTriggersNotInDeeperLevels{
	...
	all h1: HierarchicalStates, r1:Regions | 
		r1 in getAllRegionsInSameAndDeeperLevels[h1.h_contains] => disj [h1.name, r1.name] // For all regions deeper inside a composite state without regions
	all r1, r2: Regions | 
