...
pred noSameTriggersInSameLevels{
	...
	all disj s1, s2: States - allContainedNodes & States | disj[s1.name, s2.name] // In the outermost level, no states have same names
	all rs1: RegionsStates, disj r1, r2: Regions & rs1.contains | disj[r1.name, r2.name] // In a composite state, no regions have same names
}
