...
fact{
	...
	all disj s1, s2: State, c1: CompositeState | s1 in c1.contains && s2 in c1.contains && (s1.named + s2.named) != none  => s1.named != s2.named // In a composite state, no states have same names
	all disj s1, s2: State, r1: Region | s1 in r1.contains && s2 in r1.contains && (s1.named + s2.named) != none => s1.named != s2.named // In a region, no states have same names
	all disj s1, s2: State | s1 not in (Region.contains + CompositeState.contains) && s2 not in (Region.contains + CompositeState.contains) && (s1.named + s2.named) != none  => s1.named != s2.named // In the outermost level, no states have same names
	...
	all c1: CompositeState, r1:Region | r1 in getAllRegionInSameAndDeeperLevel[c1] => c1.named & r1.named = none // For all regions deeper inside a composite state
