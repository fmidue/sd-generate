...
fact{
	...
	all c1: CompositeStateWithoutRegion | #(EndState & c1.contains) < 2 // In composite states, there is at most one end state.
	lone e1: EndState | e1 not in (CompositeStateWithoutRegion.contains + Region.contains) // Outside composite states and regions, there is also at most one end state.
