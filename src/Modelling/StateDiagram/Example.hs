module Modelling.StateDiagram.Example where
...
picture1
  = umlStateDiagram $
    StateDiagram
    [ CombineDiagram [a,b,c] 1 ] 12 "active" [] [1,2,2]
  where
...
slide246
  = umlStateDiagram $ StateDiagram a 86 "" (map (\x -> Connection [x] [x+1]
...
slide253
  ...
  where
    ...
    b = StateDiagram d 2 "Minuten"
        (map (\x -> Connection [x] [x+1] "after(1min)") [1..59]
        ++
        [Connection [60] [1] "after(1min)"])
        [1]
      where
...
slide257
  ...
  where
    ...
    b = CombineDiagram [e, f] 2
      where
        e = StateDiagram c 1 "Stunden"
            (map (\x -> Connection [x] [x+1] "h") [1..23]
            ++
            [Connection [24] [1] "h"]) [1]
          where
            c = map (\x -> InnerMostState x (show(x-1)) "") [1..24]
        f = StateDiagram d 2 "Minuten"
            (map (\x -> Connection [x] [x+1] "after(1min)") [1..59]
            ++
            [Connection [60] [1] "after(1min)"])
            [1]
...
task26a
  = umlStateDiagram $ StateDiagram [a, b, c, d] 23 ""
...
implicitExitTask85 :: UMLStateDiagram String Int
implicitExitTask85
  = umlStateDiagram $
    StateDiagram [Fork 1, a] 35 ""
    [ Connection [1] [2, 1, 1, 1, 1] ""
...
flatCase7
  = let
    ...
    in
    umlStateDiagram $
    StateDiagram [isA,isB,isC,sd1,sd2] 0 ""
      [ Connection [1] [4] "a"
      , Connection [4,1] [2] "c"
      , Connection [2] [3] "d"
      , Connection [3] [4,1] "e"
